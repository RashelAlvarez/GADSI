...
if(isset($_GET['id'])){ //si viene un ID entro al codigo
	...
	$wsqli="delete from auditoria where idauditoria='$id'"; //variable mas importante
	$result = $linki->query($wsqli);
	//echo($wsqli);

//####################################################################################
	if($linki->errno){ //errno: Numero de Error
		$_SESSION['mensaje']="<b>Error!!</b> LA AUDITORIA NO SE PUEDE ELIMINAR";
		$_SESSION['tm']=0;
	}else{
		$_SESSION['mensaje']="<b>Error!!</b> LA AUDITORIA FUE ELIMINADO con exito";
		$_SESSION['tm']=1;
	}
//####################################################################################
