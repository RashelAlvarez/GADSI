...
if(isset($_GET['id'])){ //si viene un ID entro al codigo
	...
	$wsqli="delete from nivel_riesgo where idnivelriesgo='$id'"; //variable mas importante
	$result = $linki->query($wsqli);
	//echo($wsqli);

//####################################################################################
	if($linki->errno){ //errno: Numero de Error
		$_SESSION['mensaje']="<b>Error!!</b> No se puede eliminar";
		$_SESSION['tm']=0;
	}else{
		$_SESSION['mensaje']="Fue eliminado con exito";
		$_SESSION['tm']=1;
	}
//####################################################################################
